...
- 新增设备端设置屏(`main/demo_settings.c`、`main/app_settings.c`),含 9 个设置项——语言、占用阈值、平滑系数、量程、告警模式、音量、亮度、Ping 间隔、重新标定——改动实时应用并持久化到 NVS。新增完整的中英双语上屏 i18n(`main/ui_i18n.c`,默认中文),并配套一款子集 Noto Sans SC(OFL)CJK 位图字体(`main/lv_font_echo_cjk_16.c`、`lv_font_echo_cjk_20.c`,由 `lv_font_conv` 生成,`tools/check_cjk_font.py` 自查),覆盖屏幕实际用到的约 74 个汉字;顺带修复了字体缺少 `CONFIG_LV_USE_FONT_COMPRESSED` 标志导致的方块字(tofu)问题,以及设置屏的切换语言卡死问题。
- 新增声光告警:EMPTY→OCCUPIED 的绊线跳变现在会触发蜂鸣(音量可调;告警模式关/单次/连续,连续模式在占用期间每 1.5 秒响一次),并发送 BLE 告警通知。屏闪告警曾尝试过,但根据用户反馈已去掉。
- 把固定阈值的占用判定换成自标定、抗尖峰的绊线检测(`main/occupancy.c`):进入 ECHO 模式先采集约 7 秒空场 CSI 运动样本,用其中值与中值绝对偏差(MAD)推算占用阈值;随后占用判定基于滑动窗口中值(抗尖峰),搭配独立的进入/离开去抖计时器,输出 OCCUPIED/EMPTY 及持续秒数,并可随时重新标定(OK 双击或 BLE `calib` 命令)。由 `tests/test_occupancy.c` 覆盖。
- 把 BLE GATT 外设改为常驻:开机即启动并广播(`AIPassport-CSI`),独立于当前模式运行。STATUS(`e2e90002`)与 CONTROL(`e2e90003`)现覆盖完整的 ECHO/RADAR 契约(协议 v2)——STATUS 新增 `mode`、占用(`occ`/`occ_s`)、绊线触发计数器 `alert`、RADAR 各类型设备计数;CONTROL 在 `start`/`stop`/`ftm`/`ping_ms` 之外新增 `mode`(远程切模式)、`sens`、`occ_th`、`alert`、`alert_mode`、`vol`、`bright`、`calib`。
- 将全彩 ECHO HUD 设计实装到设备 UI,为物理圆角屏加入圆角安全区(内容内收 10px,四角约 18px 留白),并把菜单精简为两种被动感知设备:ECHO(Wi-Fi CSI 在场/运动)与 RADAR(设备发现)。移除数独、扫雷两个游戏以及 Display/Button/Audio/Battery 硬件自检页,连同其源文件、模型、`puzzle_ui`/`ui_pixel`/`ui_pixel_math` 辅助与对应主机测试(`tools/validate.sh --static` 同步更新);新增共享的 `main/ui_echo.c` 为菜单、ECHO、RADAR 三屏提供 HUD 配色与控件。RADAR 计数新增 UNKNOWN(`?`)项与设备总数,使随机 MAC 的 STA 被体现而非「消失」。BSP(`components/bsp`)保持不变。
- 新增 RADAR,被动设备发现菜单模式(`main/demo_discovery.c`):混杂模式嗅探 1–13 信道(不关联任何网络),32 条 LRU 设备表,粗粒度分类(AP 来自 beacon、手机/电脑/物联网按 OUI 表匹配、随机 MAC 归为未知),以彩色的各类型计数、累计总数与设备清单呈现。
- 新增 `feature/wifi-csi-sensing` 分支:ECHO,被动 Wi-Fi CSI 感知菜单模式(`main/demo_csi.c`)。ESP32-C3 作为 STA 连接用户自己的路由器,ping 网关以产生稳定 CSI 流(约 10 Hz),在设备上计算 EWMA 运动分数,并把每条记录以 CSV 行从 USB-Serial-JTAG 串流出来,同时启用 Wi-Fi/BLE 软件共存。可选 FTM 测距(ESP32-C3 作为发起端)在 AP 非 responder 时优雅降级为 N/A。纯逻辑(`main/csi_metric.c`、`main/csi_proto.c`)由 host 测试 `tests/test_csi_metric.c`、`tests/test_csi_proto.c` 覆盖,已接入 `tools/validate.sh --static`。Wi-Fi 凭证经 menuconfig 设置、绝不入库。可在 ESP-IDF 5.5.3 上编译通过(app 约占 3 MB 分区的 1.54 MB)。见 [`docs/development/wifi-csi-sensing.zh_CN.md`](development/wifi-csi-sensing.zh_CN.md)。
