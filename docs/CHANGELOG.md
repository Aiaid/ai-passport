...
- Added an on-device Settings screen (`main/demo_settings.c`, `main/app_settings.c`) with nine items — language, occupancy threshold, smoothing, scale, alert mode, volume, brightness, ping interval, and recalibrate — applied live and persisted to NVS. Added full zh/en on-screen i18n (`main/ui_i18n.c`) defaulting to Chinese, with a subset Noto Sans SC (OFL) CJK bitmap font (`main/lv_font_echo_cjk_16.c`, `lv_font_echo_cjk_20.c`, generated via `lv_font_conv`, checked by `tools/check_cjk_font.py`) covering the ~74 Chinese characters used on screen; fixed the font's missing `CONFIG_LV_USE_FONT_COMPRESSED` flag (previously rendered as tofu) and a language-switch hang in the Settings screen along the way.
- Added light-and-sound alerts: an EMPTY→OCCUPIED tripwire crossing now sounds a buzzer (adjustable volume; alert mode off/once/continuous, continuous repeating every 1.5 s while occupied) and sends a BLE alert notification. A screen-flash alert was tried and removed based on user feedback.
- Replaced the fixed-threshold occupancy check with a self-calibrating, spike-robust tripwire (`main/occupancy.c`): entering ECHO samples ~7 s of empty-room CSI motion and derives an occupancy threshold from its median and median absolute deviation (MAD); occupancy is then decided from a sliding-window median (anti-spike) with separate enter/leave debounce timers, reported as OCCUPIED/EMPTY plus a duration in seconds, and can be re-calibrated on demand (OK double-click or the BLE `calib` command). Covered by `tests/test_occupancy.c`.
- Made the BLE GATT peripheral always-on: it starts at boot and advertises (`AIPassport-CSI`) independent of the active mode. STATUS (`e2e90002`) and CONTROL (`e2e90003`) now cover the full ECHO/RADAR contract (protocol v2) — STATUS reports `mode`, occupancy (`occ`/`occ_s`), the `alert` trigger counter, and RADAR per-type device counts; CONTROL accepts `mode` (remote mode switch), `sens`, `occ_th`, `alert`, `alert_mode`, `vol`, `bright`, and `calib` in addition to `start`/`stop`/`ftm`/`ping_ms`.
- Applied the full-color ECHO HUD design to the device UI, with a rounded safe-area inset (10 px content margin, ~18 px corner clearance) for the physically rounded display, and trimmed the menu to the two passive-sensing devices: ECHO (Wi-Fi CSI presence/motion) and RADAR (device discovery). Removed the Sudoku and Minesweeper games and the Display/Button/Audio/Battery hardware self-test pages, along with their sources, models, the `puzzle_ui`/`ui_pixel`/`ui_pixel_math` helpers, and their host tests (`tools/validate.sh --static` updated accordingly); a shared `main/ui_echo.c` now provides the HUD palette and widgets for the menu, ECHO, and RADAR screens. The RADAR counter adds an UNKNOWN (`?`) tally plus a device total so randomized-MAC stations are reflected rather than hidden. BSP (`components/bsp`) is unchanged.
- Added RADAR, a passive device-discovery menu mode (`main/demo_discovery.c`): promiscuous-mode sniffing across channels 1-13 with no association, a 32-entry LRU device table, and coarse classification (AP from beacons, phone/PC/IoT by OUI lookup, Unknown for randomized MACs) shown as color-coded per-type counts plus a running total and device list.
- Added the `feature/wifi-csi-sensing` branch: ECHO, a passive Wi-Fi CSI sensing menu mode (`main/demo_csi.c`). The ESP32-C3 connects as a STA to the user's own router, pings the gateway for a steady CSI stream (~10 Hz), computes an on-device EWMA motion score, and streams each record as a CSV line over USB-Serial-JTAG, with Wi-Fi/BLE software coexistence enabled. Optional FTM ranging (the ESP32-C3 as initiator) degrades gracefully to N/A when the AP is not an FTM responder. Pure logic (`main/csi_metric.c`, `main/csi_proto.c`) is covered by host tests `tests/test_csi_metric.c` and `tests/test_csi_proto.c`, wired into `tools/validate.sh --static`. Wi-Fi credentials are set via menuconfig and never committed. Builds on ESP-IDF 5.5.3 (app ~1.54 MB of the 3 MB partition). See [`docs/development/wifi-csi-sensing.md`](development/wifi-csi-sensing.md).
