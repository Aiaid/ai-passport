...
- Applied the full-color ECHO HUD design to the device UI and trimmed the menu to the two passive-sensing devices: ECHO (Wi-Fi CSI presence/motion) and RADAR (device discovery). Removed the Sudoku and Minesweeper games and the Display/Button/Audio/Battery hardware self-test pages, along with their sources, models, the `puzzle_ui`/`ui_pixel`/`ui_pixel_math` helpers, and their host tests (`tools/validate.sh --static` updated accordingly); a shared `main/ui_echo.c` now provides the HUD palette and widgets for the menu, ECHO, and RADAR screens. The RADAR counter adds an UNKNOWN (`?`) tally plus a device total so randomized-MAC stations are reflected rather than hidden. BSP (`components/bsp`) is unchanged.
- Added the `feature/wifi-csi-sensing` branch: ECHO, a passive Wi-Fi CSI sensing menu mode (`main/demo_csi.c`). The ESP32-C3 connects as a STA to the user's own router, pings the gateway for a steady CSI stream, computes an on-device EWMA motion score, and streams each record as a CSV line over USB-Serial-JTAG. A NimBLE GATT peripheral (`main/csi_ble.c`, advertised as `AIPassport-CSI`) exposes a STATUS notify characteristic and a CONTROL write characteristic (`start`/`stop`/`ftm`/`ping_ms`) with Wi-Fi/BLE software coexistence enabled; optional FTM ranging degrades gracefully when the AP is not an FTM responder. Pure logic (`main/csi_metric.c`, `main/csi_proto.c`) is covered by host tests `tests/test_csi_metric.c` and `tests/test_csi_proto.c`, wired into `tools/validate.sh --static`. Wi-Fi credentials are set via menuconfig and never committed. Builds on ESP-IDF 5.5.3 (app ~1.19 MB of the 3 MB partition); not yet device-verified. See [`docs/development/wifi-csi-sensing.md`](development/wifi-csi-sensing.md).
