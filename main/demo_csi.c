...
static void capture_start(void)
{
    ...
    s_ftm_valid = 0;
    {
        float a = CSI_MOTION_ALPHA, sc = CSI_MOTION_SCALE;
        echo_state_get_sens(&a, &sc);  // 采用面板下发的灵敏度(默认同旧值)
        csi_motion_init(&s_filt, a, sc);
    }
...
{
    (void)arg;
    demo_radio_nvs_prepare();  // WiFi 依赖 NVS(BLE 在主循环常驻)

    // 丢弃进入本模式前遗留在共享队列里的命令。
    csi_cmd_t drain;
    while (echo_state_take_cmd(&drain)) { }
    ...
    while (!s_stop_req) {
        ...
        if (!got) vTaskDelay(pdMS_TO_TICKS(100));
    }

    // 收场:停采集(关 CSI/ping/FTM、停 WiFi)。BLE 常驻,不在此停。
    capture_stop();
